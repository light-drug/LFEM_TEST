// This file is copied from the MFEM library. For more information and source code
// availability visit https://mfem.org.

#ifndef QUEST_MEM_MANAGER_HPP
#define QUEST_MEM_MANAGER_HPP
...
// Implementation of QUEST's lightweight device/host memory manager designed to
// work seamlessly with the OCCA, RAJA, and other kernels supported by QUEST.

/// Memory types supported by QUEST.
...
extern QUEST_EXPORT const char *MemoryTypeName[MemoryTypeSize];
...
/// Class used by QUEST to store pointers to host and/or device memory.
...
private:
   ...
   template <std::size_t align_bytes, bool dummy = true> struct Alloc
   {
      ...
      static inline T *New(std::size_t)
      {
         ...
         QUEST_ASSERT(false, "overaligned type cannot use MemoryType::HOST");
         return nullptr;
      }
#else
...
/** The QUEST memory manager class. Host-side pointers are inserted into this
    manager which keeps track of the associated device pointer, and where the
    data currently resides. */
class QUEST_EXPORT MemoryManager
{
private:
   ...
   QUEST_ENZYME_INACTIVE static MemoryType host_mem_type;

   /// Device memory type set during the Setup.
   QUEST_ENZYME_INACTIVE static MemoryType device_mem_type;

   /// Allow to detect if a global memory manager instance exists.
   QUEST_ENZYME_INACTIVE static bool exists;
   ...
   QUEST_ENZYME_INACTIVE static MemoryType dual_map[MemoryTypeSize];
   ...
   QUEST_ENZYME_INACTIVE static bool configured;

   /// Host and device allocator names for Umpire.
#ifdef QUEST_USE_UMPIRE
...
private: // Static methods used by the Memory<T> class
   ...
   QUEST_ENZYME_INACTIVE static void *Register_(void *ptr, void *h_ptr,
   ...
   QUEST_ENZYME_FN_LIKE_FREE static void Delete_(void *h_ptr, MemoryType mt,
                                                unsigned flags);
   ...
   QUEST_ENZYME_FN_LIKE_DYNCAST static void *ReadWrite_(void *h_ptr,
                                                       MemoryType h_mt, MemoryClass mc,
                                                       size_t bytes, unsigned &flags);

   QUEST_ENZYME_FN_LIKE_DYNCAST static const void *Read_(void *h_ptr,
                                                        MemoryType h_mt,  MemoryClass mc,
                                                        size_t bytes, unsigned &flags);

   QUEST_ENZYME_FN_LIKE_DYNCAST static void *Write_(void *h_ptr, MemoryType h_mt,
                                                   MemoryClass mc,
                                                   size_t bytes, unsigned &flags);
   ...
   QUEST_ENZYME_INACTIVE static MemoryType GetDeviceMemoryType_(void *h_ptr,
                                                               bool alias);

   /// Return the type the of the host memory.
   QUEST_ENZYME_INACTIVE static MemoryType GetHostMemoryType_(void *h_ptr);
...
public:
   ...
   int PrintPtrs(std::ostream &out = QUEST::out);

   /// Prints all aliases known by the memory manager
   /// returning the number of printed pointers
   int PrintAliases(std::ostream &out = QUEST::out);
                            ...
                            bool own, bool valid_host, bool valid_device)
{
   ...
   h_ptr = h_ptr_;
   capacity = size;
   QUEST_ASSERT(IsHostMemory(h_mt),"");
   QUEST_ASSERT(valid_host || valid_device,"");
   const size_t bytes = size*sizeof(T);
...
{
   QUEST_ASSERT(0 <= offset, "invalid offset = " << offset);
   QUEST_ASSERT(0 <= size, "invalid size = " << size);
   QUEST_ASSERT(offset + size <= base.capacity,
...
#elif QUEST_HYPRE_VERSION < 23100
...
#else // HYPRE_USING_GPU is defined and QUEST_HYPRE_VERSION >= 23100
...
{
   QUEST_ASSERT((flags & VALID_HOST) && !(flags & VALID_DEVICE),
...
{
   QUEST_ASSERT((flags & VALID_HOST), "invalid host pointer access");
   return h_ptr[idx];
}
...
{
   QUEST_ASSERT(Empty() || (flags & VALID_HOST), "invalid host pointer access");
   return h_ptr;
}
...
inline void Memory<T>::Sync(const Memory &other) const
{
   ...
   {
      QUEST_ASSERT(h_ptr == other.h_ptr &&
...
inline void Memory<T>::SyncAlias(const Memory &base, int alias_size) const
{
   ...
   QUEST_ASSERT(!(flags & Registered) || (base.flags & Registered),
               "invalid base state");
...
{
   QUEST_VERIFY(src.capacity>=size && capacity>=size, "Incorrect size");
   if (size <= 0) { return; }
   if (!(flags & Registered) && !(src.flags & Registered))
   {
      if (h_ptr != src.h_ptr)
      {
         QUEST_ASSERT(h_ptr + size <= src.h_ptr || src.h_ptr + size <= h_ptr,
                     "data overlaps!");
...
{
   QUEST_VERIFY(capacity>=size, "Incorrect size");
   if (size <= 0) { return; }
   if (!(flags & Registered))
   {
      if (h_ptr != src)
      {
         QUEST_ASSERT(h_ptr + size <= src || src + size <= h_ptr,
                     "data overlaps!");
         std::memcpy(h_ptr, src, size*sizeof(T));
      }
      // *this is not registered, so (flags & VALID_HOST) must be true
...
inline void Memory<T>::CopyToHost(T *dest, int size) const
{
   ...
   if (!(flags & Registered))
   {
      if (h_ptr != dest)
      {
         QUEST_ASSERT(h_ptr + size <= dest || dest + size <= h_ptr,
                     "data overlaps!");
...
extern QUEST_EXPORT MemoryManager mm;

} // namespace QUEST

#endif // QUEST_MEM_MANAGER_HPP
