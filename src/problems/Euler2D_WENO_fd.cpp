...
Euler2D_WENO_FD::Euler2D_WENO_FD(const TensorMesh2D* mesh2D,
                                 const EX_TVDRK* rk_table,
                                 const int x_order)
  : mesh2D_(mesh2D), rk_table_(rk_table), x_order_(x_order), gamma_(1.4e0), CFL_(0.4e0),
    num_equations_(4)
{
...
{
  const Matrix& cc = mesh2D_->getCellCenter();
  const int Nx = mesh2D_->getxDiv();
  const int Ny = mesh2D_->getyDiv();
  u_.assign(num_equations_, Matrix::Zero(Ny, Nx));

  for (int j = 0; j < Ny; ++j) {
    for (int i = 0; i < Nx; ++i) {
      const int id = mesh2D_->CellIndex(i, j);
      const real_t x = cc(0, id);
      const real_t y = cc(1, id);
      ...
      u_[1](j, i) = rho * vx;
      u_[2](j, i) = rho * vy;
      u_[3](j, i) = pre / (gamma_ - 1.e0) + 0.5e0 * rho * (vx * vx + vy * vy);
    }
  }

  const int stages = rk_table_->getstages();
  u_stages_.assign(stages, std::vector<Matrix>(num_equations_, Matrix::Zero(Ny, Nx)));
  Lu_stages_.assign(stages, std::vector<Matrix>(num_equations_, Matrix::Zero(Ny, Nx)));
}
...
real_t Euler2D_WENO_FD::max_wave_speed(const std::vector<Matrix>& u) const
{
  ...
  Matrix p = (gamma_ - 1.e0) * (u[3].array() - 0.5e0 * rho.array() * (vx.array().square() + vy.array().square()));
...
{
  const real_t rho = u(0), vx = u(1) / rho, vy = u(2) / rho;
...
void Euler2D_WENO_FD::Lu_compute(const std::vector<Matrix>& u, const real_t& Trun,
                                 std::vector<Matrix>* Lu)
{
  const int Nx = mesh2D_->getxDiv(), Ny = mesh2D_->getyDiv();
  const real_t hx = mesh2D_->gethx(), hy = mesh2D_->gethy();
  const int k = (x_order_ - 1) / 2;
  const real_t alpha = max_wave_speed(u);

  std::vector<Matrix> fluxx(num_equations_, Matrix::Zero(Ny, Nx + 1));
  std::vector<Matrix> fluxy(num_equations_, Matrix::Zero(Ny + 1, Nx));

  auto state = [&](const std::vector<Matrix>& a, int j, int i, bool periodic)->Vector {
    int ii = i, jj = j;
    if (periodic) {
      ii = (ii % Nx + Nx) % Nx;
      jj = (jj % Ny + Ny) % Ny;
    ...
    if (ii >= 0 && ii < Nx && jj >= 0 && jj < Ny) {
      for (int m = 0; m < 4; ++m) U(m) = a[m](jj, ii);
      return U;
    }
    const Matrix& cc = mesh2D_->getCellCenter();
    int cid = mesh2D_->CellIndex(std::max(0, std::min(ii, Nx - 1)), std::max(0, std::min(jj, Ny - 1)));
    real_t x = cc(0, cid), y = cc(1, cid);
    if (ii < 0) x = mesh2D_->getx1() - (0.5e0 + (-ii - 1)) * hx;
    if (ii >= Nx) x = mesh2D_->getx2() + (0.5e0 + (ii - Nx)) * hx;
    if (jj < 0) y = mesh2D_->gety1() - (0.5e0 + (-jj - 1)) * hy;
    if (jj >= Ny) y = mesh2D_->gety2() + (0.5e0 + (jj - Ny)) * hy;
    const real_t rho = rho_bc(x, y, Trun);
    const real_t vx = vx_bc(x, y, Trun);
    const real_t vy = vy_bc(x, y, Trun);
    const real_t p = pre_bc(x, y, Trun);
    U(0) = rho; U(1) = rho * vx; U(2) = rho * vy; U(3) = p / (gamma_ - 1.e0) + 0.5e0 * rho * (vx * vx + vy * vy);
    return U;
  };

  const bool periodic = mesh2D_->IsPeriodBoundary();
  ...
  for (int j = 0; j < Ny; ++j) {
    for (int i = 0; i <= Nx; ++i) {
      ...
      for (int m = 0; m < num_equations_; ++m) fluxx[m](j, i) = fhat(m);
    }
  }
  ...
  for (int i = 0; i < Nx; ++i) {
    for (int j = 0; j <= Ny; ++j) {
      ...
      Vector fhat = (x_order_ == 3) ? (weno3_left_biased(gp) + weno3_right_biased(gm))
                                    : (weno5_left_biased(gp) + weno5_right_biased(gm));
      for (int m = 0; m < num_equations_; ++m) fluxy[m](j, i) = fhat(m);
    }
  }

  Lu->assign(num_equations_, Matrix::Zero(Ny, Nx));
  for (int m = 0; m < num_equations_; ++m) {
    (*Lu)[m] = -((fluxx[m].rightCols(Nx) - fluxx[m].leftCols(Nx)) / hx
              + (fluxy[m].bottomRows(Ny) - fluxy[m].topRows(Ny)) / hy);
  }
}
...
real_t Euler2D_WENO_FD::rho_init(const real_t& x, const real_t& y) const { return 1.e0 + 0.2e0 * std::sin(x + y); }
...
real_t Euler2D_WENO_FD::rho_bc(const real_t& x, const real_t& y, const real_t& t) const { return 1.e0 + 0.2e0 * std::sin(x + y - 2.e0 * t); }
...
Euler2D_WENO_FD_period::Euler2D_WENO_FD_period(const TensorMesh2D* mesh2D,
                                               const EX_TVDRK* rk_table,
                                               const int x_order)
