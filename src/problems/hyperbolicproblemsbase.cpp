...
const std::vector<Matrix>& HyperbolicProblems1DBase::getumodal() const { return u_modal_; }

void HyperbolicProblems1DBase::setcfl(const real_t& cfl) { cfl_ = cfl; }

HyperbolicProblems2DBase::HyperbolicProblems2DBase(const TensorMesh2D* mesh2D,
                                                   const fespace2D* fe,
                                                   const EX_TVDRK* rk_table)
  : mesh2D_(mesh2D), fe_(fe), rk_table_(rk_table)
{
  num_equations_ = fe_->getnum_equations();
  const int& stages = rk_table_->getstages();
  ...
  flux_int_.resize(num_equations_, Matrix::Zero(nq1d, intboundaryNum));
  flux_ext_.resize(num_equations_, Matrix::Zero(nq1d, extboundaryNum));
}

const std::vector<Matrix>& HyperbolicProblems2DBase::getumodal() const { return u_modal_; }

void HyperbolicProblems2DBase::setcfl(const real_t& cfl) { cfl_ = cfl; }
