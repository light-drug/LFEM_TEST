...
{
public:
  Euler2D_WENO_FD(const TensorMesh2D* mesh2D,
  ...
  virtual void Lu_compute(const std::vector<Matrix>& u, const real_t& Trun,
...
{
public:
  Euler2D_WENO_FD_period(const TensorMesh2D* mesh2D,
                         const EX_TVDRK* rk_table,
                         const int x_order);
