...
/*
  ./bin/poissonsolver2D_period_example -m 4 -nx 20 -ny 20 -basis 2 -plot 0 -poi 3
*/

int main(int argc, char *argv[])
{ 
  std::string title = "Accuracy test for Poisson equation using DG method !";
  std::string OutputDir = "result/DG/Poisson2D_DG_period_acctest";
  ...
  for (int i = 0; i < m; i++)
  {
    ...
    mesh2D.init();
    mesh2D.generateextNeighbors_period(boundarytype);
    ...
    poi_param.C11 = C11;
    poi_param.C12 = C12;
    // std::cout << " PoissonSolver2DParameter " << std::endl;
    // PAUSE();
    QUEST::Poisson_acctest_2D_period poi(&fe, poi_param);
    poi.init(poisoltype, poitol);
    // std::cout << " Poisson_acctest_2D_period " << std::endl;
    // PAUSE();
